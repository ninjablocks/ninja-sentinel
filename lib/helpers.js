...
function sendAlertSms(req,to,zone) {
  ...
  req.phone.sendSms(to,'This is Ninja Sentinel. You have an alert in zone... '+zone.name,{},function(sms) {
    ...
    console.log("User "+req.body.id+' Zone '+zone.id+' resulted in sms sent to '+to);
  });
...
function makeAlertCall(req,to,zone) {
  ...
  console.log("User "+req.body.id+' Zone '+zone.id+' calling '+to);
  req.phone.makeCall(to, null, function(call) {
    ...
    call.on('answered', function(reqParams, res) {
        ...
        console.log("User "+req.body.id+' Zone '+zone.id+' call answered by '+to);
...
function sendAlertEmail(req,to,zone) {
  ...
  req.mailer.sendMail({
    ...
    function(err, result) {
      ...
      else console.log("User "+req.body.id+' Zone '+zone.id+' resulted in email sent to '+to);
  });
